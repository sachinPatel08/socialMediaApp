...
const [followers,setFollowers] = useState([])
    ...
    }).then(res=>res.json()).then(data => setFollowers(data))
...
followers.map((e,i)=>{
    return(
        <>
        <Div>
        <Card className="m-3 " body>{e.User.userName} 
        <Button onClick={()=>{unfollow(e.followerId)}} className="ml-0"  variant="light">unfollow</Button>
        </Card>
